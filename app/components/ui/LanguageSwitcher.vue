...
<template>
  <nav
    class="relative inline-grid grid-cols-2 rounded-full border border-slate-200 bg-slate-100 p-1 shadow-inner shadow-slate-900/4"
    :aria-label="t('common.labels.language')"
  >
    <span
      aria-hidden="true"
      class="absolute left-1 top-1 h-[calc(100%-0.5rem)] w-[calc(50%-0.25rem)] rounded-full bg-slate-800 shadow-sm transition-transform duration-200 ease-out"
    ...
    <NuxtLink
      ...
      :hreflang="item.code === 'mk' ? 'mk-MK' : 'en-US'"
